...
+(void)makeRotationToView: (UIView*)aView withAngle:(CGFloat)angle withDuration:(CGFloat) duration withRepeatCount:(float) aRepeatCount delegate:(id) aDelegate{
	CATransform3D rotationTransform = CATransform3DMakeRotation(angle, 0, 0, 1.0);
	...
	[aView.layer addAnimation:rotationAnimation forKey:@"rotation"];
...
+(void) makeTranslationToView:(UIView*) aView WithX:(CGFloat) x withY:(CGFloat) y withZ:(CGFloat) z withDuration:(CGFloat) duration delegate:(id) aDelegate{
	CATransform3D translationTransform = CATransform3DMakeTranslation(x, y, z);
	CABasicAnimation *translationAnimation;
	translationAnimation = [CABasicAnimation animationWithKeyPath:@"transform"];
	translationAnimation.delegate = aDelegate;
	translationAnimation.toValue = [NSValue valueWithCATransform3D:translationTransform];
	translationAnimation.duration = duration;
	translationAnimation.removedOnCompletion = NO;
	translationAnimation.fillMode = kCAFillModeForwards;
	[aView.layer addAnimation:translationAnimation forKey:@"translation"];
}
